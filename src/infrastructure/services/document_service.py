...
class MavatDocumentFetcher:
    ...
    def fetch_plan_documents(
        self, plan_id: str, mavat_url: Optional[str] = None
    ) -> List[PlanDocument]:
        """
        ...
        try:
            ...
            if cached_docs:
                logger.info(
                    f"Found {len(cached_docs)} cached documents for plan {plan_id}"
                )
                return cached_docs
    ...
    def download_document(
        self, document_url: str, plan_id: str, doc_type: str = "pdf"
        ...
        try:
            ...
            subdir = (
                "pdfs"
                if doc_type == "pdf"
                else "images" if doc_type == "image" else "dwgs"
            )
    ...
    def _get_extension_from_url(self, url: str, doc_type: str) -> str:
        ...
        return {"pdf": ".pdf", "image": ".jpg", "dwg": ".dwg"}.get(doc_type, ".bin")
