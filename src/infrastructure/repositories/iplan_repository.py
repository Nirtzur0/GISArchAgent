...
class IPlanGISRepository(IPlanRepository):
    ...
    def consume_last_error(self) -> Optional[Dict[str, str]]:
        """
        Return and clear the most recent boundary error metadata.

        This is intentionally optional (duck-typed) so application services can
        consume degraded external-dependency signals without changing the
    ...
    def get_by_id(self, plan_id: str) -> Optional[Plan]:
        ...
        try:
            ...
            results = self._query_service(
                "planning", where, limit=1, operation="get_by_id"
            )
            ...
            self._record_error("get_by_id", e)
            return None
    ...
    def search_by_keyword(self, keyword: str, limit: int = 10) -> List[Plan]:
        ...
        try:
            ...
            results = self._query_service(
                "planning", where, limit=limit, operation="search_by_keyword"
            )
            return [self._map_to_entity(r) for r in results]

        except Exception as e:
            logger.error(f"Failed to search by keyword {keyword}: {e}")
            self._record_error("search_by_keyword", e)
            return []
    ...
    def search_by_status(
        ...
        try:
            ...
            results = self._query_service(
                "planning", where, limit=limit, operation="search_by_status"
            )
            return [self._map_to_entity(r) for r in results]

        except Exception as e:
            logger.error(f"Failed to search by status: {e}")
            self._record_error("search_by_status", e)
            return []
    ...
    def get_plan_image(self, plan_id: str) -> Optional[bytes]:
        ...
        try:
            ...
            service_url = self.SERVICES["planning"].rsplit("/", 1)[
                0
            ]  # Remove layer number
            ...
            response = self._session.get(
                export_url, params=params, timeout=self.timeout
            )
            ...
            self._record_error("get_plan_image", e)
            return None
    ...
    def _query_service(
        ...
        *,
        ...
        if not service_url:
            ...
            self._record_error(
                operation or f"query_{service_key}",
                ValueError(f"Unknown service: {service_key}"),
            )
            return []
        ...
        try:
            ...
            self._record_error(operation or f"query_{service_key}", e)
            return []
