...
class PlanUploadService:
    ...
    def analyze_uploaded_plan(
        self, file_data: BinaryIO, filename: str, max_results: int = 5
        ...
        try:
            ...
            if file_ext == "pdf":
                ...
                image_paths = self.document_processor.convert_to_images(
                    type(
                        "obj",
                        (object,),
                        {"document_type": "pdf", "local_path": str(file_path)},
                    )()
                )
            ...
            plan_id = (
                f"upload:{start_time.strftime('%Y%m%d_%H%M%S')}:{Path(filename).name}"
            )
            ...
            matches = self.regulation_repo.search_by_text(
                query=search_text, limit=max_results
            )
    ...
    def _estimate_zone_type(self, vision_analysis: VisionAnalysis) -> Optional[str]:
        ...
        description_lower = (
            vision_analysis.description.lower() if vision_analysis.description else ""
        )
